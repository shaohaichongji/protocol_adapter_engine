...
void Check(std::string_view json, pae::protocol_lab_ui::SchemaDispatchStatus expected,
  ...
  if (result.status != expected) {
    std::cerr << "SCHEMA_DISPATCH_FAIL case=" << name << " actual="
              << static_cast<int>(result.status) << " expected=" << static_cast<int>(expected)
...
int main() {
  ...
  Check(R"({"nested":{"schema_version":"0.9"}})",
        SchemaDispatchStatus::CLASSIFICATION_FAILED, "missing_root");
  Check(R"({"schema_version":9})", SchemaDispatchStatus::CLASSIFICATION_FAILED,
        "wrong_type");
  Check(R"({"schema_version":null})", SchemaDispatchStatus::CLASSIFICATION_FAILED,
        "null_type");
  ...
  Check(R"({"schema_version":"0.9",})", SchemaDispatchStatus::CLASSIFICATION_FAILED,
        "trailing_comma");
  Check(R"({"schema_version":"0.9"} // comment)",
        SchemaDispatchStatus::CLASSIFICATION_FAILED, "comment");
  Check(std::string(4U * 1024U * 1024U + 1U, 'x'),
        SchemaDispatchStatus::CLASSIFICATION_FAILED, "input_limit");
