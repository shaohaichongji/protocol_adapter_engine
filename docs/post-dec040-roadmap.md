...
实现、Windows验证与本轮已发现的P2纠错已完成，当前处于分组提交准备阶段，DEC-041尚未Commit/Push。
...
- 上述两个提交已Push；DEC-040文档收口另已本地提交为`57743bf`，尚未Push。
...
1. 固定完整记录的SUM8（逐字节求和取低8位）完整性生成/验证最小切片：已实现，待独立提交。
2. 数值转换需求细化：区分动态raw值输入、单字段比例/偏移和宿主业务多输入派生。
