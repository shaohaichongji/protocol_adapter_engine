...
这些文件保留 2026-09-14 当时的路径、Hash 和边界。用户随后已永久删除外层 `F:\PersonalWorkspace\协议解析拼接工具\Lab验证` 与 `F:\PersonalWorkspace\协议解析拼接工具\工程整理归档`；JSON 和历史正文中指向这两个目录的路径现已失效，不得据此宣称外层归档仍可访问或可恢复。
