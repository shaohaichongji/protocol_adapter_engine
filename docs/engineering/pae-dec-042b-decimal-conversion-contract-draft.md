...
中文路径修复已随`df43165`提交并Push；用户确认按代理离线验收PASS收口C3离线功能检查点，
用户人工仍NOT_EVALUATED且不阻塞本检查点，详见第22.12节；
...
推进流程见[功能检查点计划](../archive/checkpoint-delivery-plan.md)。
...
### 22.12 代理验收作为功能检查点收口依据（2026-09-08）

用户确认以代理离线验收通过作为本功能检查点收口依据。代码检查点为已提交并Push的`df43165`；
沿用[代理验收报告](agent-dec042b-c3-offline-acceptance.md)的七项场景、11条命令和35项精确检查，
不新增或改写历史执行证据。DEC-042B C3离线功能检查点据此收口。

`USER_MANUAL_ACCEPTANCE=NOT_EVALUATED`保持不变，人工清单转为后续可用性检查而非本检查点
阻塞项。此决定不是完整PAE V0.1验收、稳定公共API发布或生产准入，不关闭Oracle、Linux、
真实协议Golden、硬件、现场及性能边界。下一步先规划最小业务嵌入验证，实施仍需授权。
