...
工程入口与教学示例检查点 `657d085`、交付归集与路径治理检查点 `320a3cc` 均已提交推送。
三个外部旧根已由用户手动删除，归集目标 287 文件的长度与 SHA-256 复核一致。
具体范围见 [归集收口记录](portable-workspace-plan-20260922.md)。

当前推荐 SDK 仍为 clean `7b4205e` 五包；Lab 为 `fe1683c` 加构建时尚未提交的诊断与打包修补。
这些修补现已提交，但旧二进制并未重新构建，不能标为当前 HEAD 产物。
路径与验证边界见 [交付入口](../../deliverables/README.md)。工程整理未新增执行能力，
也不表示稳定 API/ABI、Linux、许可、正式发布或真实协议验收已完成。
